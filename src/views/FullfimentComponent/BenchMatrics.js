...
function BenchMatrix() {
  ...
  const [permissionNew, setPermissionNew] = useState([]);
    ...
    axios
      ...
      .then((resp) => {
        ...
        setPermissionNew(
          getData.filter(
            (item) =>
              item.display_name === "Historical" ||
              item.display_name === "Trending"
          )
        );
  ...
  let currentScreenName = ["Bench Metrics", "Bench Report"];
  ...
  useEffect(() => {
    if (buttonState === "Bench Report") {
      setVisible(false);
      setCheveronIcon(FaChevronCircleUp);
      getUrlPathBenchReport();
    }
  }, [buttonState]);
  ...
  const getMenus = () => {
    axios({
      ...
      data.forEach((item) => {
        console.log(item.display_name, "first");
  ...
  const getUrlPathBenchReport = () => {
    axios({
      method: "get",
      url:
        baseUrl +
        `/CommonMS/security/authorize?url=/rmg/bench&userId=${loggedUserId}`,
    }).then((res) => {
  ...
  const handleOnExport = () => {
    ...
    const filteredData = data?.filter((item, index) => index !== 1);
    if (filteredData.length > 0) {
      filteredData[0].name = filteredData[0].name.split("^&")[0];
      filteredData[0].bench = filteredData[0].bench.split("^&")[0];
      filteredData[0].cogs_count = filteredData[0].cogs_count.split("^&")[0];
      filteredData[0].fullAlloc_count =
        filteredData[0].fullAlloc_count.split("^&")[0];
      filteredData[0].innovCat1_count =
        filteredData[0].innovCat1_count.split("^&")[0];
      filteredData[0].nBCat1_count =
        filteredData[0].nBCat1_count.split("^&")[0];
      filteredData[0].noAlloc_count =
        filteredData[0].noAlloc_count.split("^&")[0];
      filteredData[0].pBCat1_count =
        filteredData[0].pBCat1_count.split("^&")[0];
    }
  ...
  return (
    <>
      <div>
        {buttonState === "Bench Report" && (
          <div>
            ...
            <div className="col-md-12">
              <div className="pageTitle">
                <div className="childOne">
                  ...
                  {/* <button
                      // className=" buttonDisplayClick"
                      key={permission[0]?.display_name}
                      className={
                        buttonState === permission[0]?.display_name.toString()
                          ? "buttonDisplayClick"
                          : "buttonDisplay"
                      }
                      onClick={() => {
                        setButtonState(permission[0]?.display_name.toString());
                        setUrlState(
                          permission[0]?.url_path.toString().replace(/::/g, "/")
                        );
                        // console.log(button.display_name);
                      }}
                    >
                    ...
                    <button
                      // className=" buttonDisplayClick"
                      key={permission[3]?.display_name}
                      className={
                        buttonState === permission[3]?.display_name.toString()
                          ? "buttonDisplayClick"
                          : "buttonDisplay"
                      }
                      onClick={() => {
                        setButtonState(permission[3]?.display_name.toString());
                        setUrlState(
                          permission[3]?.url_path.toString().replace(/::/g, "/")
                        );
                        // console.log(button.display_name);
                      }}
                    >
                      {permission[3]?.display_name}
                    </button>
                  </div> */}
                </div>
            ...
            {deleteMessage ? (
              <div className="statusMsg success">
                <span className="errMsg">
                  <BiCheck size="1.4em" /> &nbsp;Action Item Deleted
                  successfully
                </span>
              </div>
            ) : (
        ...
        {/* {buttonState === "Bench Report" && <BenchReport urlState={urlState} />} */}
        {buttonState === "Trending" && (
          <ResourceOverview
